...
# Quando ok, lint:
pnpm --filter @impactxlab/design-system lint   # quando setar
...
- [ ] **Imports do `@impactxlab/design-system`:** não pode haver erro de "module not found" — pnpm symlink funciona
...
- Build da `ds-education` (`pnpm --filter @impactxlab/design-system build`) — sem tsup config ainda; próxima sessão
