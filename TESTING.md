...
# Quando ok, lint:
pnpm --filter @impactx/ds-education lint   # quando setar
...
- [ ] **Imports do `@impactx/ds-education`:** não pode haver erro de "module not found" — pnpm symlink funciona
...
- Build da `ds-education` (`pnpm --filter @impactx/ds-education build`) — sem tsup config ainda; próxima sessão
