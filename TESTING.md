...
# Quando ok, lint:
pnpm --filter @camillodev/ui lint   # quando setar
...
- [ ] **Imports do `@camillodev/ui`:** não pode haver erro de "module not found" — pnpm symlink funciona
...
- Build da `ds-education` (`pnpm --filter @camillodev/ui build`) — sem tsup config ainda; próxima sessão
