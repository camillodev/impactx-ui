...
> Contrato declarativo pro bot que vai gerar código consumindo `@camillodev/ui`.
...
- **Preciso de botão?** → `<Button>` de `@camillodev/ui`. Ver `rules/components/button.md`.
...
## 7. Componentes disponíveis em `@camillodev/ui`
