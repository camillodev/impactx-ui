...
> Contrato declarativo pro bot que vai gerar código consumindo `@impactxlab/design-system`.
...
- **Preciso de botão?** → `<Button>` de `@impactxlab/design-system`. Ver `rules/components/button.md`.
...
## 7. Componentes disponíveis em `@impactxlab/design-system`
